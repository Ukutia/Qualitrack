...
describe('crear temáticas con documentos recién cargados', () => {
  it.each(['admin'])('permite crear al %s sin depender de chunks, vectorización o autor de la carga', async (role) => {
  ...
  it('mantiene la validación cuando no existen documentos activos', async () => {
    ...
    const res = response();
    await createTopic({ user: { id: 7, role: 'admin' }, body: { name: 'Calidad' } }, res, vi.fn());
    expect(res.status).toHaveBeenCalledWith(409);
    expect(prisma.topic.create).not.toHaveBeenCalled();
  });
  it('rechaza nombres inválidos antes de consultar documentos', async () => {
    const res = response();
    await createTopic({ user: { id: 7, role: 'admin' }, body: { name: 'ab' } }, res, vi.fn());
    expect(res.status).toHaveBeenCalledWith(400);
    expect(prisma.document.count).not.toHaveBeenCalled();
  });
  it('habilita ambas visualizaciones solo para el administrador', () => {
    for (const [method, path] of [['POST', '/search/semantic'], ['GET', '/topics/network'], ['POST', '/topics'], ['DELETE', '/topics/1']]) {
      expect(canAccess('admin', method, path)).toBe(true);
      expect(canAccess('user', method, path)).toBe(false);
